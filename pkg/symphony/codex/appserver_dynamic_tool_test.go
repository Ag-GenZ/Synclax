...
func TestAppServer_GitHubGraphQL_ToolCallProtocol(t *testing.T) {
	var gotAuth string
...
while IFS= read -r line; do
  ...
  case "$count" in
    ...
    4)
      printf '%s\n' '{"id":3,"result":{"turn":{"id":"turn-1"}}}'
      printf '%s\n' '{"id":202,"method":"item/tool/call","params":{"name":"github_graphql","arguments":{"query":"query Viewer { viewer { login } }"}}}'
...
`
	...
	_, err = srv.RunTurn(ctx, session, workspace, "GH-1: test", "hello", nil)
	if err != nil {
		t.Fatalf("RunTurn: %v", err)
	}

	if gotAuth != "Bearer x" {
		t.Fatalf("expected GitHub Authorization header Bearer x, got %q", gotAuth)
	}
	...
	trace := string(traceBytes)
	if !strings.Contains(trace, `"name":"github_graphql"`) {
		t.Fatalf("expected github_graphql tool call in trace, trace=%s", trace)
	}
}
