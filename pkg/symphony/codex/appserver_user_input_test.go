...
func TestAppServer_ApprovalRequests_UseSupportedDecisions(t *testing.T) {
	tmp := t.TempDir()
	traceFile := filepath.Join(tmp, "codex.trace")
	fakeCodex := filepath.Join(tmp, "fake-codex")
...
while IFS= read -r line; do
  ...
  case "$count" in
    ...
    4)
      ...
      printf '%s\n' '{"id":201,"method":"file/requestApproval","params":{"reason":"need file edit approval"}}'
...
`
	...
	_, err = srv.RunTurn(ctx, session, workspace, "approval", "hello", nil)
	if err != nil {
		t.Fatalf("RunTurn: %v", err)
	}
	...
	trace := string(traceBytes)
	if !strings.Contains(trace, `"decision":"decline"`) {
		t.Fatalf("expected approval decline decision in trace, got=%s", trace)
	}
	if strings.Contains(trace, `"decision":"reject"`) {
		t.Fatalf("unexpected legacy reject decision in trace, got=%s", trace)
	}
}
