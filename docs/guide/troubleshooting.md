...
- tracker 配置缺失：
  - Linear：`tracker.api_key` / `tracker.project_slug`
  - GitHub：`tracker.token` / `tracker.project_owner` / `tracker.project_number` / `tracker.repository`
- `codex.command` 为空或运行环境缺少 `codex` 命令

最短修复路径：

1. `cp WORKFLOW.md.example WORKFLOW.md`
2. 设置你所选 tracker 的 token（`LINEAR_API_KEY` 或 `GITHUB_TOKEN`）
3. 在 `WORKFLOW.md` 中补齐该 tracker 的必填字段
...
- 确认 tracker 中确实存在 `Todo` / `In Progress` 的 issue
- GitHub Projects v2 模式下，确认 `tracker.state_field` 对应的是现有 single-select 字段，且 option 名称与工作流状态名一致
