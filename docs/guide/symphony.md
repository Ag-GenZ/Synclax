...
- [Symphony SPEC 解析（Reference）](../reference/symphony-spec)
...
### 3.1 Tracker：Linear 或 GitHub Projects v2

要让 Symphony 真正开始派发任务，你至少需要配置一个受支持的 tracker：
...
- `tracker.state_field` 必须是目标 Project v2 中已存在的 single-select 字段
- 该字段 option 名称必须直接使用工作流状态字符串
- v1 只支持单个 repository
...
- attempt **成功**且 issue 仍 active → 进入 continuation retry（短延迟再次执行）
...
- hook 要**可重复执行**（幂等）
- hook 要**尽量快**，必要时调大 `hooks.timeout_ms`
...
- 本机 debug：需要时再开 `server.port` 或 `http_port`
