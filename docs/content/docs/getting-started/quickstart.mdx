---
title: Quickstart
description: 把 Synclax 跑起来并在 Web Console 里看到它在工作。
---
...
如果你在排障时需要直接用 `curl` 验证后端 API，请看 [HTTP API Reference（Advanced）](/docs/reference/http-api)。
...
- 需要配置详解（WORKFLOW / 环境变量）→ [Configuration](/docs/getting-started/configuration)
- 需要理解 orchestrator 行为与 phase → [Symphony](/docs/concepts/symphony)
- 想先理解理念与架构 → [哲学理念](/docs/concepts/philosophy) / [系统架构](/docs/concepts/architecture)
