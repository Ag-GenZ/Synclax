...
tracker:
  ...
  token: $GITHUB_TOKEN
  project_owner: Ag-GenZ
  project_number: 1
  repository: Ag-GenZ/Synclax
  state_field: Status
  active_states:
    - Todo
    - In Progress
...
hooks:
  after_create: |
    git clone --depth 1 git@github.com:Ag-GenZ/Synclax.git .
  before_run: |
    anclax install
    make gen
    cd web && pnpm install
...
You are working on a GitHub Project v2 issue `{{ issue.identifier }}`
...
## Prerequisite: GitHub access is available

The agent should be able to talk to GitHub using the configured tracker token and available GitHub tooling. If required GitHub access is missing, stop and report it as a blocker.

## Project-specific guardrails

- **Never run `build` in the frontend** (`cd frontend && pnpm build` or similar) — it will crash the machine.
- The repo has a `frontend/` (pnpm) and a `backend/` (pnpm) directory. Dependencies are already installed by the `before_run` hook; do not re-run full installs unless needed.
...
1. Find or create a single persistent workpad comment for the issue:
   ...
   - Ignore resolved comments while searching; only active/unresolved comments are eligible to be reused.
...
5. Ensure the workpad includes a compact environment stamp at the top as a code fence line:
   ...
   - Example: `devbox-01:/home/dev/.cache/symphony_workspaces/WIB-42@7bdde33`
...
3. Treat every actionable reviewer comment (human or bot), including inline review comments, as blocking until addressed or explicitly pushed back with justification.
...
- If a non-GitHub required tool is missing, or required non-GitHub auth is unavailable, move the ticket to `Human Review` with a short blocker brief in the workpad that includes: what is missing, why it blocks, and exact human action needed to unblock.
...
8. Attach PR URL to the issue (prefer attachment; use workpad comment only if attachment is unavailable).
   - Ensure the GitHub PR has label `synclax` (add it if missing).
...
- Required PR metadata is present (`synclax` label).
